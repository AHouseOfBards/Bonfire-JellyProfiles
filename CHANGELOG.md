...
- A Bonfire switch could show a setting the server never accepted, with nothing on screen to say the save had failed.
- Switching profiles on a light theme flashed a black screen.
- A switch that went wrong left the screen blank for four seconds. It is now under two.
...
- The maximum profiles box accepted a negative number, and nothing enforced an upper limit.
...
**Nothing changes for you in this build.** It exists so the next ones break less often.

**Internal**

- Bonfire's test suite now lives in the repository and runs on every change. It was kept outside it, which is how 1.5.2 and 1.5.3 shipped a client script that did not start.
- Every change is now built and tested automatically before it can be merged. Previously only finished releases were.
